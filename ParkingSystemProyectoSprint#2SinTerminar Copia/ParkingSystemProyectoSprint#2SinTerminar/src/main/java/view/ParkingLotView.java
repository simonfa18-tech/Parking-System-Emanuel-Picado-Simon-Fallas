...
public class ParkingLotView extends JInternalFrame implements ActionListener {
    ...
    private void exitVehicleAndCalculateFee() {
        if (controller.getAllParkingLots().isEmpty()) {
            JOptionPane.showMessageDialog(this, "No parking lots registered");
            return;
        }
        StringBuilder info = new StringBuilder("Available parking lots:\n\n");
        for (ParkingLot parkingLot : controller.getAllParkingLots()) {
            info.append("ID: ").append(parkingLot.getId()).append(" - ").append(parkingLot.getName()).append("\n");
        }
        int parkingLotId;
        try {
            parkingLotId = Integer.parseInt(JOptionPane.showInputDialog(info + "\nEnter the parking lot ID"));
        ...
        if (parkingLot == null || parkingLot.getVehicles().isEmpty()) {
            JOptionPane.showMessageDialog(this, "This parking lot has no vehicles");
            return;
        }
        StringBuilder vehiclesInfo = new StringBuilder("Vehicles in the parking lot:\n\n");
        for (Vehicle v : parkingLot.getVehicles()) {
            vehiclesInfo.append("- ").append(v.getPlate()).append("\n");
        }
        String plate = JOptionPane.showInputDialog(vehiclesInfo + "\nEnter the vehicle plate to remove:");
        if (plate == null || plate.isEmpty()) {
            JOptionPane.showMessageDialog(this, "You must enter a plate");
            return;
        }
        try {
